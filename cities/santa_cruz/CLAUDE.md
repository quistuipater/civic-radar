...
**This is a fork of Ventura Civic Radar, brought to operational parity 2026-07-10.** The engine (FastAPI + SQLAlchemy + Jinja2 dashboard + a Python worker as the ingestion scheduler, orchestrated via `docker-compose.yml`: postgres/pgvector, ollama, api, worker) is unchanged from Ventura's working Phase 0 build. 8 real Santa Cruz sources are seeded and ingesting live: County Board of Supervisors (PrimeGov), County Planning Commission (a bespoke new connector, `app/ingestion/scc_planning_search.py` — a legacy classic-ASP full-text search tool, not PrimeGov, unlike Ventura where BOS/PC shared one platform), City of Santa Cruz agendas (another bespoke new connector, `app/ingestion/onbase_agenda.py` — Hyland OnBase, not CivicPlus), and 4 NetFile campaign-finance/SEI feeds (county `SCCO` + city `CRUZ`, genuinely separate feeds) plus County Elections. A real end-to-end run produced 227 documents and 30+ meetings, all successfully parsed and AI-classified/summarized via real Ollama calls on `madhatter.local`. Two real gaps remain, both investigated and confirmed rather than just unstarted: no incident-level crime data exists for either the City PD or County Sheriff (checked both ArcGIS Hubs directly), and meeting audio isn't wired in (county's Granicus podcast feed is empty; both county and city video streams are CloudFront-gated, confirmed via a direct request with a matching Referer header still returning 403 — see README's "Known gaps" section, not something to retry casually). Note: `gpt-oss:20b` produced garbled output on `madhatter.local` when tested from Ventura Civic Radar (MXFP4/kernel issue) — `OLLAMA_TRIAGE_MODEL`/`OLLAMA_ANALYSIS_MODEL` are pinned to `llama3.1:8b` until that's debugged; re-verify if pointing at a different Ollama instance.

Common commands:
- `docker compose up -d postgres && docker compose run --rm api python scripts/init_db.py` — create schema
- `docker compose run --rm api python scripts/seed_sources.py && docker compose run --rm api python scripts/seed_prompts.py` — seed source registry (8 real Santa Cruz sources) + versioned prompts (both idempotent)
...
- `backend/tests/`: `docker compose run --rm api pytest`. 567 tests (535 inherited from Ventura at fork time + 32 new for the two Santa-Cruz-specific connectors, both at 100% coverage), ~99% overall. Runs against a real `civic_radar_test` Postgres database (not sqlite — several models need pgvector/JSONB), each test isolated in a rolled-back transaction (see `join_transaction_mode="create_savepoint"` in `tests/conftest.py` — required since app code calls `db.commit()` for real; `db_session_factory` exposes the same sessionmaker directly for code like `worker.py` that opens its own `SessionLocal()` rather than taking a `db` param). Re-run after any Santa Cruz-specific change to confirm nothing regressed.

Follow the existing stack and module layout in `backend/app/` rather than introducing a different framework or reorganizing — it mirrors the PRD's architecture directly (see below). Most connectors (PrimeGov, NetFile, generic) turned out to be reusable as-is against real Santa Cruz platforms; two sources (City of Santa Cruz agendas, County Planning Commission) needed genuinely new bespoke ingestion modules because Santa Cruz's real platforms differ from Ventura's — both follow the established pattern of session-stateful, non-generic-connector ingestion already used by `meeting_audio.py`/`crime_data.py`. Check `README.md`'s "What's implemented" section before assuming a capability is "just config" — two real ones weren't.
