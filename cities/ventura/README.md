...
*This is the Ventura instantiation of the Civic Radar platform — see the
repo root's [`OVERVIEW.md`](../../OVERVIEW.md) for the shared engine,
architecture, and design principles this deployment builds on (Ventura is
also the origin: the platform's engine was built against this jurisdiction
first, then forked for Santa Cruz and Boston). This document covers only
what's actually specific to Ventura.*
...
`../../core/tests/conftest.py`) — required because application code under test calls
...
`../../core/tests/conftest.py`) exposes that same sessionmaker directly, for the rare
...
Santa Cruz and Boston — see the repo root's `OVERVIEW.md` for the full
module map. This directory (`cities/ventura/`) holds only what's actually
...
cities/ventura/
  ...
  scripts/update_triage_model.py   one-off migration script, kept for the record
