...
**This is a fork of Ventura Civic Radar, brought to partial operational status for Boston, MA on 2026-07-10.** The engine (FastAPI + SQLAlchemy + Jinja2 dashboard + a Python worker as the ingestion scheduler, orchestrated via `docker-compose.yml`: postgres/pgvector, ollama, api, worker) is unchanged from Ventura's working Phase 0 build. **5 real Boston sources are seeded and ingesting live**: City Council agendas/minutes via Legistar (`app/ingestion/legistar.py`, a new bespoke module — Legistar is a real, documented, unauthenticated REST/OData API, not CivicPlus/PrimeGov), Zoning Board of Appeal agendas/minutes via the same Legistar module (`BodyId=199`, pure config — Boston's closest structural analog to Ventura's county Planning Commission, even though Boston has no county layer for it to sit under), Boston PD crime incidents via ArcGIS (pure `AGENCY_CONFIG` addition to `app/ingestion/crime_data.py`, no new code), Massachusetts OCPF campaign-finance filings for the Mayor + City Council (`app/ingestion/connectors/ocpf.py`, plugs into the existing generic `discover()`/`CONNECTORS` dispatch since OCPF's `/reports/log` is JSON, not HTML/RSS), and Elections-department public notices (`app/ingestion/connectors/boston_public_notices.py`, also a generic-dispatch connector — each notice is its own HTML detail page, not a linked PDF, so the stock `generic.py` harvester finds nothing there). A real end-to-end run produced 101 new City Council documents across 58 meetings, 19 new Zoning Board of Appeal documents across 19 meetings, 330,012 crime incidents (first full sync), 1 real OCPF filing, and 1 real public notice. One category remains open, investigated and confirmed rather than unstarted (see README.md's "Known gaps"): meeting audio (Boston's Granicus podcast feed is genuinely populated, unlike Santa Cruz's, but the MP3s are CloudFront-gated and 403 even with a matching Referer header). Note: `gpt-oss:20b` produced garbled output on `madhatter.local` when tested from Ventura Civic Radar (MXFP4/kernel issue) — `OLLAMA_TRIAGE_MODEL`/`OLLAMA_ANALYSIS_MODEL` are pinned to `llama3.1:8b` until that's debugged; re-verify if pointing at a different Ollama instance. A standalone WhisperX transcription service (`whisperx_service/`) exists as scaffolding only; not deployed, since the meeting-audio gap above means there's nothing to transcribe yet.

Common commands:
- `docker compose up -d postgres && docker compose run --rm api python scripts/init_db.py` — create schema
- `docker compose run --rm api python scripts/seed_sources.py && docker compose run --rm api python scripts/seed_prompts.py` — seed source registry (5 real Boston sources) + versioned prompts (both idempotent)
...
- `backend/tests/`: `docker compose run --rm api pytest`. 561 tests (536 inherited from Ventura at fork time + 25 new for the three Boston-specific connectors, all at 100% coverage), ~99% overall. Runs against a real `civic_radar_test` Postgres database (not sqlite — several models need pgvector/JSONB), each test isolated in a rolled-back transaction (see `join_transaction_mode="create_savepoint"` in `tests/conftest.py` — required since app code calls `db.commit()` for real; `db_session_factory` exposes the same sessionmaker directly for code like `worker.py` that opens its own `SessionLocal()` rather than taking a `db` param). Re-run after any Boston-specific change to confirm nothing regressed.

Follow the existing stack and module layout in `backend/app/` rather than introducing a different framework or reorganizing — it mirrors the PRD's architecture directly (see below). Legistar and OCPF/boston_public_notices needed genuinely new ingestion code because Boston's real platforms differ from every one this project had seen in CA; crime data and the Zoning Board of Appeal source (same Legistar module as City Council, different `BodyId`) were pure-config wins. Check `README.md`'s "What's implemented" section before assuming a Boston capability doesn't exist yet — five real ones do.
