...
*This is the Boston instantiation of the Civic Radar platform — see the
repo root's [`OVERVIEW.md`](../../OVERVIEW.md) for the shared engine,
architecture, and design principles this deployment builds on. This
document covers only what's actually specific to Boston.*
...
`../ventura/` in this same monorepo), **brought to partial operational
status for Boston, MA on 2026-07-10.** The engine (ingestion connectors,
parsing, AI layer, dashboard, alerting) is generic and already works
end-to-end against Ventura and, as of the same day, Santa Cruz
(`../santa_cruz/`). **5 real Boston sources are
...
- **Test suite**: pytest — see "Running tests" below for current counts.
  Fixture defaults (`../../core/tests/conftest.py`) use "City of Boston" rather than
...
`../../core/tests/test_crime_data.py`, `../../core/tests/conftest.py`) — the same adjustment
...
Ventura and Santa Cruz — see the repo root's `OVERVIEW.md` for the full
module map. This directory (`cities/boston/`) holds only what's actually
specific to this deployment:

```
cities/boston/
  seed_sources.py        the source registry — bind-mounted into the container
                          over ../../core/scripts/seed_sources.py
  city_settings.py        crime AGENCY_CONFIG (Boston Police Department) —
                           bind-mounted over ../../core/app/city_config.py
  docker-compose.yml      builds against ../../core; Boston's ports/DB name/env vars
  .env.example, README.md (this file), CLAUDE.md
  tests/test_legistar.py  the bespoke Legistar connector test

../../core/app/
  models.py            SQLAlchemy models — the schema from prd.md section 11
  ingestion/            fetch + archive + per-source connectors (generic and bespoke,
                        all cities — Boston's legistar.py, connectors/ocpf.py,
                        connectors/boston_public_notices.py live here too)
