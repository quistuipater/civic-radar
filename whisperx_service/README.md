...
**Not deployed for Santa Cruz yet.** This directory was forked from Ventura
...
for the working unit file/deployment steps to copy). No Santa Cruz
meeting-audio source has been identified yet (see this repo's README TODO
section), so there's nothing to transcribe until one is. If/when you do wire
one in: **do not reuse port 8091 or the `~/repos/whisperx` directory** on
madhatter for a second instance -- either point Santa Cruz's
`WHISPERX_BASE_URL` at the existing Ventura instance (fine if it's not
saturated) or deploy a second copy on a different port with its own systemd
unit name.
