...
`where` filtering on it silently no-opped). No Santa Cruz agency has been
added below yet -- see this file's AGENCY_CONFIG and README's TODO section.
"""
...
# Keyed by Source.agency (exact string match). No Santa Cruz-area agency
# added yet -- find the local police department's/sheriff's public ArcGIS
# FeatureServer (if one exists) and add an entry here matching the pattern
# below before seeding a crime_data_feed Source for it. Example shape,
# generalized from Ventura Civic Radar's real entries (field names below
# are illustrative, not real -- inspect the actual FeatureServer's fields
# via its /query endpoint before filling this in):
#
# AGENCY_CONFIG = {
