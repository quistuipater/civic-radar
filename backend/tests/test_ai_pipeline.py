"""Tests for run_ai_pipeline's orchestration/gating logic. The four
sub-steps (embed_document_chunks, extract_agenda_items, classify_document,
summarize_document) are monkeypatched as spies here -- this module's own
job is just deciding *whether* and *in what order* to call them, which is
what these tests verify, not re-testing each sub-step's internal behavior
(already covered in test_embed.py, test_agenda_items.py, and the router
tests for classify/summarize).
"""
...
class TestRunAiPipeline:
    ...
    def test_embed_and_extract_agenda_items_always_run_before_the_classification_gate(self, db, monkeypatch):
        # These two should run even if the document already has a
        ...
        assert calls == ["embed", "extract_agenda_items"]
    ...
    def test_classify_and_summarize_run_when_no_prior_classification_exists(self, db, monkeypatch):
        ...
        assert calls == ["embed", "extract_agenda_items", "classify", "summarize"]
