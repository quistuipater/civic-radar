...
class TestParseDocument:
    ...
    def test_embedded_nul_bytes_are_stripped_before_chunks_are_stored(self, db, tmp_path, monkeypatch):
        ...
        # seen live on a real 900+ page Santa Cruz budget packet (same
        # shared parsing code as this repo) whose PDF extraction produced a
        # NUL byte partway through. parse_file() is mocked here since
        # reproducing that exact PDF-extraction artifact isn't practical in
        # a unit test; what matters is that parse_document sanitizes
        # whatever parse_file() hands back before it reaches the DB.
