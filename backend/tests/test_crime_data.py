...
unnoticed. They exercise the validation logic against a synthetic agency
config registered per-test via monkeypatch, so they don't break if the real
production AGENCY_CONFIG entries are restructured.
"""
...
class TestIngestCrimeSource:
    ...
    def test_empty_response_on_full_refresh_source_is_flagged_not_silently_ok(self, db, archive_root, monkeypatch):
        ...
        # poll (as both real Ventura agencies do) -- for those, 0 features
        # back is a real anomaly, not "nothing new today".
        _register_test_agency(monkeypatch)
        source = make_source(db, name="Test PD", agency=TEST_AGENCY, url="https://example.invalid/pd")
