> **FORK NOTE (Boston Civic Radar):** This document was written for
> Ventura Civic Radar and inherited as-is when this repo was forked for
> Boston, MA. The architecture below (schema, pipeline shape, AI
...
> and should be re-derived for Boston via real source research (see
> README.md's TODO section) rather than treated as a requirement for this
> fork. Massachusetts's civic-government structure also differs from
> California's in ways that matter here — Boston is a consolidated
> city/county government (no independent Suffolk County government since
> 1999), and campaign-finance disclosure runs through the state's OCPF
> rather than county filing officers the way NetFile-based sources have
> served every CA fork so far. Nothing below has been edited for Boston yet.
