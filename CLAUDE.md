...
- `backend/tests/` (pytest, ~93% coverage as of 2026-07-08): `docker compose run --rm api pytest`. Runs against a real `civic_radar_test` Postgres database (not sqlite — several models need pgvector/JSONB), each test isolated in a rolled-back transaction (see `join_transaction_mode="create_savepoint"` in `tests/conftest.py` — required since app code calls `db.commit()` for real; `db_session_factory` exposes the same sessionmaker directly for code like `worker.py` that opens its own `SessionLocal()` rather than taking a `db` param). Covers ingestion (all connectors/fetchers), alerting/scoring/heuristics, the full REST router layer, issue matching, OCR/parsing (extract.py/service.py, 100% — PDF logic is tested against a mocked pdfplumber rather than real PDF fixtures, since the actual thing worth protecting is our OCR-cap/page.close() logic, not pdfplumber itself), the full AI orchestration layer (pipeline/classify/summarize/embed/agenda_items/ollama_client, all 100% — note `OLLAMA_BASE_URL` resolves to a genuinely reachable `madhatter.local` in this environment, so every Ollama-touching test explicitly monkeypatches `is_available()`/`generate_json()`/`embed()` rather than assuming unavailability), and the worker loop (99% — scheduling/batching/crash-isolation logic, with `ingest_source`/`parse_document`/`run_ai_pipeline`/etc. treated as spies since each has its own tests). Remaining gaps are pre-existing partial ones with no history of live bugs: `dashboard.py` (50%), `export/markdown.py` (64%), `export/digest.py` (79%), `routers/crime_incidents.py` (69%).
