...
Follow the existing stack and module layout in `backend/app/` rather than introducing a different framework or reorganizing — it mirrors the PRD's architecture directly (see below). Legistar, OCPF, and boston_public_notices needed genuinely new ingestion code because Boston's real platforms differ from every one this project had seen in CA; crime data was a pure-config win. Check `README.md`'s "What's implemented" section before assuming a Boston capability doesn't exist yet — four real ones do.
