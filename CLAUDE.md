...
- `backend/tests/` (pytest, ~83% coverage as of 2026-07-08): `docker compose run --rm api pytest`. Runs against a real `civic_radar_test` Postgres database (not sqlite — several models need pgvector/JSONB), each test isolated in a rolled-back transaction (see `join_transaction_mode="create_savepoint"` in `tests/conftest.py` — required since app code calls `db.commit()` for real). Covers ingestion (all connectors/fetchers), alerting/scoring/heuristics, the full REST router layer, issue matching, and OCR/parsing (extract.py/service.py, 100% — PDF logic is tested against a mocked pdfplumber rather than real PDF fixtures, since the actual thing worth protecting is our OCR-cap/page.close() logic, not pdfplumber itself). Not yet covered: the AI orchestration layer (`ai/pipeline.py`, `ai/agenda_items.py`, `ai/embed.py` — needs live/mocked Ollama) and the worker loop.
