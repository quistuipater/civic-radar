...
brought to operational parity 2026-07-10** — 8 real sources seeded and
...
- **`prd.md` is still Ventura's as-written.** Its architecture (schema,
  ...
  name is stale — see the fork note at the top of that file.
...
test adjustments as at fork time (see `tests/test_crime_data.py`,
`tests/conftest.py`). Re-run after making any further changes to confirm
nothing regressed.
...
backend/
  app/
    ...
    ingestion/            fetch + archive + per-source connectors
      onbase_agenda.py     City of Santa Cruz agendas (Hyland OnBase, bespoke session flow)
      scc_planning_search.py  County Planning Commission (legacy ASP search tool, bespoke session flow)
  ...
  scripts/
    ...
    seed_sources.py        seed the source registry (8 real Santa Cruz sources, see above)
